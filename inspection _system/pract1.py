...
def draw_boxes(img, results):
    ...
    low_score_classes = []
    raspberry_pico_reference = [94, 62, 252, 135]
    pico_area_threshold = (raspberry_pico_reference[2] - raspberry_pico_reference[0]) * (raspberry_pico_reference[3] - raspberry_pico_reference[1]) * (2 / 3)
    global_fail = False
    ...
    for obj in results['objects']:
        ...
        # RASPBERRY PICO 크기 검사
        if class_name == "RASPBERRY PICO":
            pico_area = (box[2] - box[0]) * (box[3] - box[1])
            if pico_area < pico_area_threshold:
                cv2.putText(img, "BROKEN PICO", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                global_fail = True
    ...
    # 상태 표시 추가
    defect_status = "FAIL" if global_fail else analyze_defects(results)
...
def analyze_defects(results):
    ...
    if len(missing_classes) == 0 and hole_count >= 3:
        return "PASS"
    elif len(missing_classes) == 1 or any(count > 1 for count in class_counts.values()):
        return "Inspection"
    else:
        return "FAIL"
